...
- Keep `xpack/styles.ts` stylesheet orchestration reliability behavior in tested core helpers where possible. The current Sass/lightningcss build uses `createStyleCompileQueue` from `xpack/styles-core.ts`, defaults `STYLES_CONCURRENCY` to `5`, and drains queued work in one-shot mode so late Sass/lightningcss failures still affect the process exit code.
...
8. For stylesheet build reliability changes, keep the concurrency parsing, queue drain behavior, and failed-task continuation covered in `xpack/styles-core.test.ts`; validate the CLI shell with `bun run styles` or `bun inte`.
9. For generated asset or sourcemap determinism changes, add tests that compare CRLF input to LF-normalized output, cover source-map `sourcesContent` escape normalization, and prove binary inputs are not rewritten.
10. Prefer removing genuinely unreachable code (with a one-line comment explaining the narrowing) over force-testing it through `as never` casts.
11. Run the narrowest impacted suites before widening validation.
12. Run `bun run test:ci` before considering the change complete.
13. For integration-output determinism, run `bun inte` and verify `{VITE_INTE_PATTERN_DIR}` plus `{VITE_INTE_ASSET_DIR}` have no content diff with `git diff --quiet -- {VITE_INTE_PATTERN_DIR} {VITE_INTE_ASSET_DIR}`. On Windows, `git status` can still list LF-normalized text files under `text=auto`; trust the content diff check.
14. If the change expands what can be tested, widen the enforced coverage scope in `vitest.config.ts` and update `docs/test-coverage.md` and `references/current-state.md`.
15. If local developer tooling changes, preserve the `simple-git-hooks` `pre-push` gate that runs `bun run typecheck` after install.
16. If the change touches CI or merge rules, verify the frontend workflow files still run `bun run test:ci` before build, deploy, or integration steps.
17. If GitHub branch protection must change, note that repository files are not enough and GitHub authentication/settings are required.
